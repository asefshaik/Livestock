...
export default function CameraScreen({ navigation, route }) {
    ...
    return (
      <SafeAreaView style={styles.centered}>
        ...
        <Text style={styles.permMessage}>
          LiveHub needs camera access to scan livestock.
        </Text>
  ...
  const displayLabel = isAuto
    ...
    : typeLabel;
  const statusText =
    liveDetection?.status_text ??
    (isRecording ? `Scanning...` : `Ready to scan ${isAuto ? "" : typeLabel}`);
        ...
        <View style={styles.progressBar} pointerEvents="none">
          <View
            style={[
              ...
              {
                width: `${Math.min(
                  (validCountRef.current / MIN_VALID_FRAMES) * 100,
                  100
                )}%`,
              },
            ]}
          ...
          <Text style={styles.progressLabel}>
            Keep {typeLabel} in frame ({validCountRef.current}/
            {MIN_VALID_FRAMES} frames)
          </Text>
        </View>
      )}
